...
function Login({ setUser }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Hand the user up to App so the protected pages update.
  ...
  return (
    <section className="flex min-h-screen w-full items-center justify-center px-4 py-8">
      <div className="w-full max-w-md rounded-xl border border-(--border) bg-(--background) p-6 shadow-(--shadow) sm:p-8">
        <h1 className="mb-1 text-3xl font-semibold text-(--text-h)">
          Log in
        </h1>
        ...
        >
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="mt-2 rounded-md bg-blue-600 px-4 py-2.5 font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        <button
          ...
          className="w-full rounded-md border border-(--border) px-4 py-2.5 font-medium transition hover:bg-black/5"
        >
          Continue with Auth0
        </button>
